...
function StrListReverse(const AStrList: TStringList): TStringListEnumReverse;
begin
  Result := TStringListEnumReverse.Create(AStrList);
end;
...
begin
  ...
  try
    ...
    for Str in StrListReverse(StrList) do
      Writeln(Str);
  finally
    FreeAndNil(StrList);
  end;
